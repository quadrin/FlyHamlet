/* Run: node --test tests/test_mb_worker.cjs. No browser or dependencies.
...
const workerSource = fs.readFileSync(path.join(__dirname, '../site/mb-worker.js'), 'utf8');
...
function runner(options = {}) {
  ...
  class FakeExperiment {
    constructor(conn, data, cells, seed, config = {}, prepared = null) {
      this.prepared = prepared; this.cells = cells;
      this.conn = conn;
      this.seed = seed;
      this.dtMs = 0.1;
      this.steps = 0;
  ...
  const context = vm.createContext({
    performance: {now: () => clock}, crypto: webcrypto, Uint32Array, Uint8Array, URL,
    importScripts: () => {},
    ...
    FlyHamletMushroomBody: {MushroomBodyExperiment: options.Experiment || FakeExperiment, DEFAULTS: {slowWeightScale: 0.3}, validateAnnotation: () => {}},
    fetch: async () => ({ok: true, json: async () => (options.annotation || {format: 'flyhamlet-mushroom-body-v1', n: 2})}),
    FlyHamletSequence: {
...
test('the annotation is loaded once with the graph and handed to every experiment', async () => {
  const worker = runner({annotation: {format: 'flyhamlet-mushroom-body-v1', n: 2, fixture: true}});
  await worker.dispatch({type: 'init', seed: 3, autoplay: false});
  await worker.dispatch({type: 'new', seed: 4, autoplay: false});
  assert.equal(worker.loads, 1);
  assert.deepEqual(worker.instances[0].cells, {format: 'flyhamlet-mushroom-body-v1', n: 2, fixture: true});
  assert.equal(worker.instances[1].cells, worker.instances[0].cells);
});

test('actual mushroom-body engine matches direct execution through all three conditions and phases', async () => {
  const {MushroomBodyExperiment} = require('../site/mb-model.js');
  ...
  const connectome = {n, indptr: new Uint32Array(indptr), indices: new Uint32Array(indices), weights: new Float32Array(weights)};
  const data = {config: {sim: {dt_ms: 1, v_rest_mV: -52, v_reset_mV: -52, v_thresh_mV: -45, tau_mem_ms: 20, tau_syn_ms: 5,
    t_refractory_ms: 2.2, delay_ms: 2, poisson_weight_mV: 68.75, dtype: 'float32', background: {enabled: false}}},
    targets: {eye_L: [0], eye_R: [1]}, layout: Array.from('abcdefghijklmnopqrstuvwxyz ')};
  const cells = {format: 'flyhamlet-mushroom-body-v1', n, kenyonCells: Array.from({length: 16}, (_, i) => 8 + i),
    mbons: Array.from({length: 16}, (_, i) => 24 + i), uniglomerularPNs: [0, 1, 2, 3, 4, 5, 6, 7]};
  const options = {phrase: 'to', trainEpisodes: 2, diagnosticEpisodes: 1, recallEpisodes: 1, maxDecisions: 4, cueMs: 10, gapMs: 5, cueRateHz: 1000};
  const worker = runner({Experiment: MushroomBodyExperiment, annotation: cells, load: () => ({connectome, manifest: data}), prepare: prepareSlowGraph});
  await worker.dispatch({type: 'init', seed: 451, speed: 100, options});
  worker.drain();
  const complete = worker.sent.find(message => message.type === 'complete');
  assert(complete, 'the real engine must complete across conditions, phases and learning');
  const slow = prepareSlowGraph(connectome, 0.3);
  const hash = require('node:crypto').createHash('sha256').update(Buffer.from(slow.graph.weights.buffer)).digest('hex');
  assert.equal(complete.result.slowGraph.scaledWeightsSha256, hash);
  const direct = new MushroomBodyExperiment(connectome, data, cells, 451, options, {graph: slow.graph, metadata: {...slow.metadata, scaledWeightsSha256: hash}});
  while (!direct.done) direct.step();
  assert.deepEqual(complete.result, direct.result());
  assert.equal(complete.result.episodes.length, 3);
