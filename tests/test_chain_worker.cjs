/* Run: node --test tests/test_chain_worker.cjs. No browser or dependencies.
...
const workerSource = fs.readFileSync(path.join(__dirname, '../site/chain-worker.js'), 'utf8');
...
function runner(options = {}) {
  ...
  const context = vm.createContext({
    ...
    FlyHamletChain: {ChainExperiment: options.Experiment || FakeExperiment, DEFAULTS: {slowWeightScale: 0.1}},
    FlyHamletSequence: {
...
test('actual chain engine matches direct execution through all three conditions and phases', async () => {
  const {ChainExperiment} = require('../site/chain-model.js');
  ...
  const options = {phrase: 'to', trainEpisodes: 1, diagnosticEpisodes: 1, recallEpisodes: 1, maxDecisions: 4, cueMs: 10, gapMs: 5, poolCount: 8, cueRateHz: 1000};
  const worker = runner({Experiment: ChainExperiment, load: () => ({connectome, manifest: data}), prepare: prepareSlowGraph});
  await worker.dispatch({type: 'init', seed: 451, speed: 100, options});
  worker.drain();
  const complete = worker.sent.find(message => message.type === 'complete');
  assert(complete, 'the real engine must complete across conditions, phases and fits');
  ...
  const direct = new ChainExperiment(connectome, data, 451, options, {graph: slow.graph, metadata: {...slow.metadata, scaledWeightsSha256: hash}});
