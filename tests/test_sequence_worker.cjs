/* Run: node --test tests/test_sequence_worker.cjs. No browser or dependencies.
...
const workerSource = fs.readFileSync(path.join(__dirname, '../site/sequence-worker.js'), 'utf8');
...
const fixtureSlow = (conn, scale) => ({graph: conn, metadata: {weightScale: scale, fixture: true}});
...
function runner(options = {}) {
  ...
  class FakeExperiment {
    constructor(conn, data, seed, config = {}, prepared = null) {
  ...
  const context = vm.createContext({
    performance: {now: () => clock}, crypto: webcrypto, Uint32Array, Uint8Array,
    importScripts: () => {},
    ...
    FlyHamletSequence: {SequenceExperiment: options.Experiment || FakeExperiment, DEFAULTS: {slowWeightScale: 0.1},
      prepareSlowGraph: (conn, scale) => {
        preparations++;
        return options.prepare ? options.prepare(conn, scale) : fixtureSlow(conn, scale);
      }},
...
test('new discards old timers and state, caches graph and comparison network, and defaults back to 1x', async () => {
  const worker = runner();
  ...
  assert.equal(worker.preparations, 1, 'the rescaled graph is reused across fresh neural sessions');
  assert.equal(worker.instances[0].conn, worker.instances[1].conn);
  assert.equal(worker.instances[0].prepared, worker.instances[1].prepared);
  assert.match(worker.instances[0].prepared.metadata.scaledWeightsSha256, /^[a-f0-9]{64}$/);
  const ready = worker.sent.filter(message => message.type === 'ready');
  assert.deepEqual(ready.map(message => [message.seed, message.generation]), [[1, 1], [2, 2]]);
  await worker.dispatch({type: 'new', seed: 3, autoplay: false, options: {slowWeightScale: 0.5}});
  assert.equal(worker.preparations, 2, 'a different weight scale needs its own comparison network');
  assert.equal(worker.instances[2].prepared.metadata.weightScale, 0.5);
});
...
test('new and pause during preparation share one preparation and start only the newest run', async () => {
  ...
  const slow = fixtureSlow(graph, 0.1);
  ...
  await worker.dispatch({type: 'pause'});
  assert.equal(worker.instances.length, 0, 'no experiment may start with incomplete preparation');
  resolvePreparation(slow);
  await Promise.all([first, second]);
  assert.equal(worker.preparations, 1);
  assert.equal(worker.loads, 1);
  assert.equal(worker.instances.length, 1);
  assert.equal(worker.instances[0].seed, 16);
  assert.equal(worker.instances[0].prepared.graph, slow.graph);
  assert.equal(worker.timers.size, 0, 'pause is respected across asynchronous preparation');
  const progress = worker.sent.filter(message => message.type === 'progress').map(message => message.message);
  assert(progress.some(message => /comparison network/.test(message)));
});
...
test('superseded preparation errors are suppressed and preparation can be retried', async () => {
  ...
  const entered = new Promise(resolve => { enteredPreparation = resolve; });
  const worker = runner({prepare: (conn, scale) => {
    if (++attempts === 1) { enteredPreparation(); return prepared; }
    return fixtureSlow(conn, scale);
  }});
...
test('actual sequence engine matches direct execution through all three conditions and diagnostics', async () => {
  const {SequenceExperiment, prepareSlowGraph} = require('../site/sequence-model.js');
  ...
  const options = {trainPerPair: 1, evaluationPerPair: 1, letterMs: 10, gapMs: 5, delaysMs: [5, 10], poolCount: 8, cueRateHz: 1000};
  const worker = runner({Experiment: SequenceExperiment, load: () => ({connectome, manifest: data}), prepare: prepareSlowGraph});
  ...
  const slow = prepareSlowGraph(connectome, 0.1);
  const hash = require('node:crypto').createHash('sha256')
    .update(Buffer.from(slow.graph.weights.buffer)).digest('hex');
  assert.equal(complete.result.slowGraph.scaledWeightsSha256, hash);
  const direct = new SequenceExperiment(connectome, data, 451, options, {graph: slow.graph, metadata: {...slow.metadata, scaledWeightsSha256: hash}});
  while (!direct.done) direct.step();
  assert.deepEqual(complete.result, direct.result());
  assert.equal(complete.result.fitCount, 3);
  assert.equal(complete.result.readoutFitCount, 18);
  assert.equal(worker.sent.flatMap(message => message.records || []).length, 24);
  assert(complete.result.trials.every(record => record.cueEnd.inputActive && record.snapshots.every(snapshot => !snapshot.inputActive)));
});

test('benchmark scoring reconstructs scalers, features and choices, and separates positional accuracy', () => {
  const {summarize, predict, standardize, scalerFromRows} = require('../scripts/benchmark_sequence.cjs');
  const observations = [
    {pair: 'to', first: 't', second: 'o', cueSpikes: 9, postCueSpikes: 4, snapshot: {pairPrediction: 'to', predictions: {first: {prediction: 't'}, second: {prediction: 'o'}}, silent: false, nonrestVoltageNeurons: 3, nonrestCurrentNeurons: 1, spikesSinceCueOff: 2}},
    {pair: 'ot', first: 'o', second: 't', cueSpikes: 7, postCueSpikes: 0, snapshot: {pairPrediction: 'tt', predictions: {first: {prediction: 't'}, second: {prediction: 't'}}, silent: true, nonrestVoltageNeurons: 0, nonrestCurrentNeurons: 0, spikesSinceCueOff: 0}},
    {pair: 'tt', first: 't', second: 't', cueSpikes: 5, postCueSpikes: 0, snapshot: {pairPrediction: 'to', predictions: {first: {prediction: 't'}, second: {prediction: 'o'}}, silent: true, nonrestVoltageNeurons: 0, nonrestCurrentNeurons: 0, spikesSinceCueOff: 0}},
    {pair: 'oo', first: 'o', second: 'o', cueSpikes: 5, postCueSpikes: 0, snapshot: {pairPrediction: 'oo', predictions: {first: {prediction: 'o'}, second: {prediction: 'o'}}, silent: true, nonrestVoltageNeurons: 0, nonrestCurrentNeurons: 0, spikesSinceCueOff: 0}}
  ];
  const score = summarize(observations, record => record.snapshot);
  assert.equal(score.accuracy, 0.5);
  assert.equal(score.balancedAccuracy, 0.5);
  assert.equal(score.firstAccuracy, 0.75);
  assert.equal(score.secondAccuracy, 0.75);
  assert.equal(score.silentTrials, 3);
  assert.equal(score.meanNonrestVoltageNeurons, 0.75);
  assert.equal(score.meanSpikesSinceCueOff, 0.5);
  assert.equal(score.meanCueSpikes, 6.5);
  const scaler = scalerFromRows([[1, 0], [1, 2]]);
  assert.deepEqual(scaler, {mean: [1, 1], scale: [0, 1]});
  assert.deepEqual(standardize(scaler, [1, 3]), [0, 2]);
  assert.deepEqual(predict([2], [1, -1, 0, 0], ['t', 'o']), {prediction: 't', scores: [2, -2]});
});
