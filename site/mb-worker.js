/* A fresh plastic mushroom-body experiment on the complete FlyWire network. The graph
 ...
 */
'use strict';
importScripts('live-model.js', 'recall-model.js', 'sequence-model.js', 'chain-model.js', 'mb-model.js', 'connectome-loader.js');
...
function begin(options) {
  experiment = new FlyHamletMushroomBody.MushroomBodyExperiment(connectome, manifest, annotation, requestedSeed, options, prepared);
...
async function loadAnnotation(manifestURL) {
  const url = manifestURL ? new URL('mushroom-body.json', manifestURL).href : 'mushroom-body.json';
  const response = await fetch(url);
  if (!response.ok) throw new Error(`Could not load mushroom-body annotations (${response.status}).`);
  const cells = await response.json();
  if (cells.format !== 'flyhamlet-mushroom-body-v1') throw new Error('Unexpected mushroom-body annotation format.');
  FlyHamletMushroomBody.validateAnnotation(cells, cells.n);
  return cells;
}
...
self.onmessage = async event => {
  ...
  try {
    if (message.type === 'init' || message.type === 'new') {
      ...
      if (!connectome) {
        if (!loading) {
          loading = Promise.all([FlyHamletConnectome.load(message.manifestURL, {progress: reportProgress}), loadAnnotation(message.manifestURL)])
            .then(([loaded, cells]) => { connectome = loaded.connectome; manifest = loaded.manifest; annotation = cells; })
            .finally(() => { loading = null; });
        }
        await loading;
      }
      if (token !== generation) return;
      const scale = message.options?.slowWeightScale ?? FlyHamletMushroomBody.DEFAULTS.slowWeightScale;
