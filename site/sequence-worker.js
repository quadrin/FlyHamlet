/* A fresh two-letter state-decoding experiment on the complete FlyWire network. The graph
 ...
 */
'use strict';
importScripts('live-model.js', 'recall-model.js', 'sequence-model.js', 'connectome-loader.js');
...
function begin(options) {
  experiment = new FlyHamletSequence.SequenceExperiment(connectome, manifest, requestedSeed, options, prepared);
...
self.onmessage = async event => {
  ...
  try {
    if (message.type === 'init' || message.type === 'new') {
      ...
      const scale = message.options?.slowWeightScale ?? FlyHamletSequence.DEFAULTS.slowWeightScale;
