/* A fresh post-cue neural-memory experiment on the complete FlyWire network. The graph is
 * cached across runs; neural state, decoder, and trial schedule are recreated.
 * Every message carries the requested seed and generation for stale-run guards.
 */
'use strict';
importScripts('live-model.js', 'recall-model.js', 'memory-model.js', 'memory-rewire.js', 'connectome-loader.js');
...
function begin(options) {
  experiment = new FlyHamletMemory.MemoryExperiment(connectome, manifest, requestedSeed, options, control);
...
self.onmessage = async event => {
  ...
  try {
    if (message.type === 'init' || message.type === 'new') {
      ...
      if (!control) {
        if (!preparing) {
          // The fixed permutation is independent of experiment seeds. Concurrent
          // New requests share this reusable preparation, then only the newest
          // generation can instantiate its mutable neural/decoder state.
          preparing = FlyHamletMemoryRewire.prepareRewire(connectome, {
            seed: 1299709,
            progress: update => reportProgress(update.message || 'Preparing rewired control', update.loaded || 0, update.total || 0),
            yieldControl: () => new Promise(resolve => setTimeout(resolve, 0))
          }).then(async prepared => {
            reportProgress('Verifying rewired control', 0, prepared.graph.indices.byteLength);
            const digest = await crypto.subtle.digest('SHA-256', prepared.graph.indices);
            const hash = Array.from(new Uint8Array(digest), byte => byte.toString(16).padStart(2, '0')).join('');
            control = {graph: prepared.graph, metadata: {...prepared.metadata, rewiredIndicesSha256: hash}};
