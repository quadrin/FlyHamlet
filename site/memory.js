/* Observer UI for the delayed-symbol assay. Stimulus and prediction labels are
 * for inspection only. No UI choice or display history reaches a model readout. */
(() => {
  'use strict';
  const $ = id => document.getElementById(`lab-${id}`);
  const {summarizeMemoryTrials, CONDITIONS, DEFAULTS} = window.FlyHamletMemory;
  const labels = {retain: 'Retain state', reset: 'Reset state', rewired: 'Rewired'};
  const colors = {retain: '#78e5ed', reset: '#ffba79', rewired: '#d0aff6'};
  ...
  let metrics = summarizeMemoryTrials([]), keys = [], layout = [], selectedLetter = null;
  ...
  function inspectLatest() {
    ...
    if (!record) {
      selectedLetter = null; text('choice', '—'); text('action', 'Waiting for held-out decisions');
      text('inspect-note', 'Only changes this display'); placeFly(); return;
    }
    const window = record.windows.find(item => item.delayMs === delay);
    selectedLetter = window.prediction; placeFly(); text('choice', symbol(window.prediction));
    text('inspect-note', `${window.populationSpikes} brain spikes · ${window.correct ? 'correct' : 'incorrect'}`);
    text('action', `Last test: ${symbol(record.cue)} → ${symbol(window.prediction)} at ${delay} ms · ${labels[record.condition]}`);
  }
  function appendLog(record) {
    if (record.phase !== 'evaluation') return;
    const entry = document.createElement('span'); entry.className = 'memory-log-entry';
    const label = document.createElement('b'); label.textContent = `${labels[record.condition]} / test ${record.phaseTrial} / cue ${symbol(record.cue)}`;
    const choices = document.createElement('span'); choices.textContent = record.windows.map(window => `${window.delayMs}:${symbol(window.prediction)}`).join('  ');
    const diagnostic = document.createElement('small'); diagnostic.textContent = `Cue visible: ${symbol(record.cueReadout.prediction)} · ${record.cueReadout.correct ? 'correct' : 'incorrect'}`;
    entry.append(label, choices, diagnostic); $('output').append(entry); $('output').scrollTop = $('output').scrollHeight;
  ...
  function drawChart(canvas, activity = false) {
    ...
    const left = 42, right = 17, top = 17, bottom = 37;
    const observed = CONDITIONS.flatMap(condition => metrics[condition].delays.filter(row => row.total).map(row => row.meanPopulationSpikes));
    const max = activity ? Math.max(1, Math.ceil(Math.max(0, ...observed) / 5) * 5) : 1;
    const X = delay => left + delay / 200 * (width - left - right);
    ...
    for (const value of [0, max / 2, max]) {
      ctx.strokeStyle = '#345566'; ctx.setLineDash([]); ctx.beginPath(); ctx.moveTo(X(0), Y(value)); ctx.lineTo(X(200), Y(value)); ctx.stroke();
      ctx.fillStyle = '#b8cbd7'; ctx.fillText(activity ? String(Number(value.toFixed(1))) : `${Math.round(value * 100)}%`, left - 7, Y(value) + 3);
    }
    if (!activity) {
      ctx.strokeStyle = '#91a6b3'; ctx.setLineDash([2, 4]); ctx.beginPath(); ctx.moveTo(X(0), Y(1 / 7)); ctx.lineTo(X(200), Y(1 / 7)); ctx.stroke();
    }
    ctx.setLineDash([]); ctx.textAlign = 'center';
    for (const delay of delays) if (width >= 450 || delay !== 10) ctx.fillText(delay, X(delay), height - 23);
    ctx.fillStyle = '#91aebb'; ctx.font = '9px monospace'; ctx.fillText('Delay after cue off (ms)', (left + width - right) / 2, height - 7);
    for (const condition of CONDITIONS) {
      const rows = metrics[condition].delays.filter(row => row.total);
      ...
      ctx.lineWidth = 1.7;
      ctx.setLineDash(condition === 'reset' ? [5, 3] : condition === 'rewired' ? [2, 3] : []); ctx.beginPath();
      rows.forEach((row, i) => { const x = X(row.delayMs), y = Y(activity ? row.meanPopulationSpikes : row.accuracy); if (!i) ctx.moveTo(x, y); else ctx.lineTo(x, y); }); ctx.stroke(); ctx.setLineDash([]);
      for (const row of rows) { ctx.beginPath(); ctx.arc(X(row.delayMs), Y(activity ? row.meanPopulationSpikes : row.accuracy), 2.5, 0, Math.PI * 2); ctx.fill(); }
    }
  }
  function updateResults() {
    metrics = result ? result.metrics : summarizeMemoryTrials(records);
    for (const condition of CONDITIONS) {
      const cue = metrics[condition].cueReadout;
      text(`${condition}-cue`, cue.total ? percent(cue.accuracy) : '—');
      text(`${condition}-cue-note`, cue.total ? `${cue.correct}/${cue.total} · ${cue.meanPopulationSpikes.toFixed(1)} brain spikes / 20 ms` : condition === 'reset' ? 'Before the reset at cue offset' : '56 held-out trials');
    }
    const fragment = document.createDocumentFragment();
    for (const delay of delays) {
      const row = document.createElement('tr'), label = document.createElement('th'); label.scope = 'row'; label.textContent = `${delay} ms`; row.append(label);
      for (const condition of CONDITIONS) {
        const metric = metrics[condition].delays.find(item => item.delayMs === delay), cell = document.createElement('td');
        if (metric.total) {
          ...
          const spikes = document.createElement('small'); spikes.textContent = `${metric.meanPopulationSpikes.toFixed(1)} spikes / window`;
          const silent = document.createElement('small'); silent.textContent = `${metric.silentTrials}/${metric.total} silent trials`;
          cell.append(accuracy, spikes, silent);
        } else cell.textContent = '—';
        row.append(cell);
      }
      fragment.append(row);
    }
    $('delay-results').replaceChildren(fragment); drawChart($('chart')); drawChart($('activity'), true); syncControls();
  }
  function updateProgress(next) {
    ...
    text('stage', next.stage === 'cue' ? 'Presenting sensory cue' : next.stage === 'readout' ? 'Measuring post-cue activity' : next.stage === 'complete' ? 'Experiment complete' : 'Cue off · neural state evolves');
    text('trial-time', `${next.trialTimeMs.toFixed(0)} / 420 ms`); $('clock').style.left = `${Math.min(100, next.trialTimeMs / 420 * 100)}%`;
  ...
  function download(data) {
    ...
    const link = document.createElement('a'); link.href = url; link.download = `flyhamlet-memory-${data.seed}${data.complete ? '' : '-partial'}.json`;
  ...
  function verifyFrozen(data) {
    ...
    return CONDITIONS.every(condition => {
      ...
      return model?.fitCount === 1 && model.readouts.length === delays.length && [model.cueReadout, ...model.readouts].every(readout =>
        readout && readout.fingerprint === readout.finalFingerprint && JSON.stringify(readout.weights) === JSON.stringify(readout.frozenWeights));
    });
  }
  function conclusion(data) {
    const rows = data.metrics.retain.delays;
    const longest = rows[rows.length - 1], first = rows[0], cue = data.metrics.retain.cueReadout;
    return `Original wiring: ${percent(cue.accuracy)} with the cue visible; ${percent(first.accuracy)} in the first post-cue window; ${percent(longest.accuracy)} at ${longest.delayMs} ms (${longest.silentTrials}/${longest.total} silent trials). Chance is 14.3%. Readouts stayed frozen. These measurements concern short-term information in this model; phrase recall from neural state remains a separate test.`;
  }
  function onMessage(event) {
    ...
    if (message.type === 'ready') {
      ...
      text('lock', 'Verified: all evaluation weights unchanged'); text('conclusion', conclusion(result)); updateResults();
    } else if (message.type === 'export') download(message.result);
  }
  function clearDisplay() {
    progress = null; selectedLetter = null; placeFly(); $('output').replaceChildren();
    ...
    text('stage', 'Awaiting trial'); text('trial-time', '0 / 420 ms'); $('clock').style.left = '0%';
    $('progress').value = 0; text('progress-text', '0 / 336 trials'); text('time', '0.00 s simulated');
    text('lock', 'Readouts have not been fitted'); text('rate', `Target speed ${$('speed').value}×`);
    text('conclusion', 'Training fits a separate readout for each condition and delay. Evaluation uses fresh noise and frozen weights. No external history, cue label, elapsed time, or earlier prediction enters a delayed readout.');
  ...
  function start() {
    ...
    text('load-message', 'Loading the full connectome. First use downloads about 49 MB, then prepares the comparison graph.');
    status('Preparing experiment', 'loading'); syncControls();
    try {
      if (!worker) {
        worker = new Worker(new URL('memory-worker.js?v=1', scriptURL)); generation = 0;
  ...
  const resize = () => { placeFly(); drawChart($('chart')); drawChart($('activity'), true); };
  if ('ResizeObserver' in window) {
    const observer = new ResizeObserver(resize); observer.observe($('chart')); observer.observe($('activity')); observer.observe($('keyboard'));
  } else window.addEventListener('resize', resize);
  $('speed').value = '1'; $('inspect').value = '50'; buildKeyboard(Array.from('rezndomstyawukihbpgqf xcvjl')); reset();
})();
